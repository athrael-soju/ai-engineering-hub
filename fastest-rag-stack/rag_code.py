...
class QdrantVDB_QB:
    ...
    def ingest_file_data(self, texts, embeddings):
        # 'texts' and 'embeddings' should be in the same order and of the same length
        from rag_code import batch_iterate  # or use the local function reference directly
        for batch_context, batch_embeddings in zip(
            batch_iterate(texts, self.batch_size), 
            batch_iterate(embeddings, self.batch_size)
        ):
...
class Retriever:
    ...
    def search(self, query):
        query_embedding = self.embeddata.generate_embedding_for_query(query)
...
class RAG:
    def __init__(self,
        ...
        self.qa_prompt_tmpl_str = (
            ...
            "answer the query in a crisp manner. If you don't know the answer, say 'I don't know!'.\n"
            "Query: {query}\n"
            "Answer: "
        )
    ...
    def generate_context(self, query):
        ...
        context_docs = [dict(data) for data in result]
        combined_prompt = []
        for entry in context_docs[:2]:
